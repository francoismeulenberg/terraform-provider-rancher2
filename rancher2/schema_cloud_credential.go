...
type CloudCredential struct {
	...
	ExoscaleCredentialConfig      *exoscaleCredentialConfig      `json:"exoscalecredentialConfig,omitempty" yaml:"exoscalecredentialConfig,omitempty"`
}
...
func cloudCredentialFields() map[string]*schema.Schema {
	s := map[string]*schema.Schema{
		...
		"exoscale_credential_config": {
			...
			MaxItems:      1,
			Optional:      true,
			ConflictsWith: getConflicts(allCloudCredentialDriverConfigFields, "exoscale_credential_config"),
			Elem: &schema.Resource{
				Schema: cloudCredentialExoscaleFields(),
			},
		},
	}
